...
export default function Sidebar({ isOpen, setIsOpen, activeTab, setActiveTab }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar Panel */}
      <aside 
        className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-zinc-800 bg-zinc-950 transition-transform lg:static lg:translate-x-0 ${
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between border-b border-zinc-800 px-6">
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col">
              <span className="font-bold tracking-wider text-zinc-100 text-sm">SECUREHAVEN</span>
              <span className="text-[9px] font-semibold text-emerald-500 uppercase tracking-widest">Zero-Trust K8s</span>
            </div>
          </div>
          <button 
            onClick={() => setIsOpen(false)} 
            className="rounded p-1 text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200 lg:hidden"
          >
        ...
        <div className="border-t border-zinc-800 p-4 bg-zinc-900/30">
          <div className="rounded-lg bg-emerald-950/20 border border-emerald-900/30 p-3">
            ...
            <p className="text-[10px] text-zinc-500 mt-1">Namespace &apos;exam&apos; is running in zero-trust isolation.</p>
          </div>
        </div>
      </aside>
    </>
